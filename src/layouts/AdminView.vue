...
const { selectedClub, isLoadingDetails: loading } = storeToRefs(clubStore);
// const { toast } = storeToRefs(userStore); // No es necesaria si las modales se manejan localmente
...
onMounted(() => {
    ...
    if (clubId) {
        clubStore.fetchClubDetails(clubId);
    }
});
</script>
        ...
        <div class="flex items-center gap-3">
          <div class="w-10 h-10 rounded-full border-2 border-accent bg-white flex items-center justify-center shrink-0">
            <span class="text-primary font-bold">
              {{ selectedClub?.details?.name?.substring(0, 2).toUpperCase() || '...' }}
            </span>
          </div>
          <div>
            <h2 class="font-semibold leading-none">{{ selectedClub?.details?.name || 'Cargando...' }}</h2>
