...
const filteredClubs = computed(() => {
  return clubs.value.filter(club => {
    const searchMatch = searchTerm.value.trim() === '' ||
      ...
      club.group_description?.toLowerCase().includes(searchTerm.value.toLowerCase());
    
    const categoryMatch = categoryFilter.value === 'all' ||
      club.group_category === categoryFilter.value;
  ...
  <div class="p-4 sm:p-6 md:p-8 bg-soft min-h-screen">
    <div class="max-w-screen-xl mx-auto">
      ...
      <div v-else-if="filteredClubs.length > 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <div v-for="club in filteredClubs" :key="club.groupId" class="bg-card border border-gray-200 rounded-xl p-5 flex flex-col shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
          <h3 class="text-xl font-bold text-primary mb-2">{{ club.group_name }}</h3>
          <p class="text-sm text-gray-600 flex-grow mb-4">{{ club.group_description || 'Este club no ha añadido una descripción.' }}</p>
          <RouterLink :to="{ name: 'ClubDetail', params: { id: club.groupId } }" class="mt-auto w-full py-2 text-center font-semibold bg-accent text-primary rounded-lg hover:opacity-90 transition-opacity focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-accent">
            Ver Club
          </RouterLink>
        </div>
      </div>
      ...
      <div v-if="!isLoadingList && filteredClubs.length === 0" class="mt-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
