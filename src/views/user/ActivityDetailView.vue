...
const { selectedActivity: activity, isLoading } = storeToRefs(activityStore);

onMounted(() => {
  activityStore.fetchActivityById(String(props.id));
});
</script>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-8 mt-6">
      ...
      <div class="md:col-span-1 space-y-6">
        ...
        <div class="bg-card border border-gray-200 p-4 rounded-xl space-y-4">
          <div class="flex items-start gap-3"><LucideIcon name="calendar-days" :size="20" class="text-accent mt-1 shrink-0"/><h4 class="font-semibold">Fecha y Hora</h4><p class="text-sm text-gray-600">{{ new Date(activity.start_date || '').toLocaleString('es-PA', { dateStyle: 'full', timeStyle: 'short' }) }}</p></div>
          ...
          <div class="flex items-start gap-3"><LucideIcon name="users-2" :size="20" class="text-accent mt-1 shrink-0"/><h4 class="font-semibold">Capacidad</h4><p class="text-sm text-gray-600">{{ activity.max_participants || '∞' }}</p></div>
        </div>
      </div>
    </div>
  </div>
