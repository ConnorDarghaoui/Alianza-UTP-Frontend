...
import type { UserEnrollDTO } from '@/services/dao/models/auth.model' 
...
const isFormComplete = computed(() => {
    return form.value.firstName && form.value.lastName && usernameOk.value && emailOk.value && pwdOk.value
})
...
async function submit(): Promise<void> {
  ...
  try {
    ...
    const payload: UserEnrollDTO = {
      ...
      // Asegurarse de que birthDate sea una cadena ISO 8601
      birthDate: new Date(form.value.birthDate).toISOString().split('T')[0], // Formato YYYY-MM-DD
      gender:     form.value.gender as 'Masculino' | 'Femenino',
      password: form.value.password
    };
    
    // 2. Llamamos directamente al método del DAO
    const [data, errorObj] = await authDAO.register(payload)
  ...
  <div class="w-full">
    ...
    <form @submit.prevent="submit" class="space-y-6">
      ...
      <div>
        ...
        <div class="space-y-4">
          <input v-model="form.username" required placeholder="Nombre de usuario" autocomplete="username" class="input-focus-effect w-full py-2.5 px-3" :class="{ 'border-red-500': form.username.length > 0 && !usernameOk }" />
          <p v-if="form.username.length > 0 && !usernameOk" class="text-xs text-red-500 -mt-2">
