...
import type { ClubMember } from '@/services/dao/models/club.model';
...
const stats = computed(() => selectedClub.value?.stats || {});
...
const totalPages = computed(() => 1); // Placeholder, la paginación real debe venir del backend
const isAllSelected = computed(() => members.value && members.value.length > 0 && selectedMembers.value.length === members.value.length);

// --- MÉTODOS ---
const filters = ref({ search: '', status: 'all', role: 'all' });

function fetchMembers() {
  clubStore.fetchClubDetails(clubId.value);
  // clubStore.fetchMemberStats(clubId.value); // Ya se obtiene con fetchClubDetails
}
...
function handleInvite() {
  ...
  // clubStore.inviteMembers(clubId.value, { emails: emailList, role: 'member' }); // Asumiendo rol por defecto
...
function exportToCSV() { 
  // clubStore.exportMembers(clubId.value); 
}
        ...
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            ...
            <tr v-else v-for="member in members" :key="member.userId" class="hover:bg-soft">
              ...
              <td class="px-6 py-4"><span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full" :class="'bg-gray-100 text-gray-800'">N/A</span></td>
            </tr>
             <tr v-if="!clubStore.isLoadingDetails && (!members || members.length === 0)"><td colspan="4" class="text-center py-8 text-gray-500">No se encontraron miembros.</td></tr>
          </tbody>
        </table>
      </div>
    </div>
